...
class Log(object):
    ...
    def echo(self, echo_string):
        ...
        click.echo(prefix + echo_string)

    def info(self, fmt, *args, **kwargs):
        self.echo(fmt.format(*args, **kwargs))

    def error(self, fmt, *args, **kwargs):
        return self.info("Error: " + click.style(fmt, fg="red"), *args, **kwargs)
...
def get_cache_dir(app_name):
    ...
    return os.path.join(
        os.environ.get("XDG_CONFIG_HOME", os.path.expanduser("~/.cache")), app_name
    )
...
class Builder(object):
    ...
    def execute(self, cmd, args=None, capture=False):
        ...
        with self.log.indented():
            cl = subprocess.Popen(
                cmdline, cwd=self.path, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            if capture:
                rv = cl.communicate()[0]
            else:
    ...
    def describe_package(self, python):
        ...
        rv = self.execute(
            python, ["setup.py", "--name", "--version", "--fullname"], capture=True
        ).strip().splitlines()
    ...
    def place_venv_deps(self, venv_path, data_dir):
        ...
        self.copy_file(os.path.join(venv_path, "virtualenv.py"), data_dir)
    ...
    def build_wheels(self, venv_path, data_dir):
        ...
        with self.log.indented():
            ...
            if self.requirements is not None:
                ...
                shutil.copy2(
                    self.requirements, os.path.join(data_dir, "requirements.txt")
                )
    ...
    def setup_build_venv(self, virtualenv):
        ...
        with self.log.indented():
            self.execute(
                self.python, [os.path.join(virtualenv, "virtualenv.py"), scratchpad]
            )
            self.execute(
                os.path.join(scratchpad, "bin", "pip"),
                ["install"]
                + self.get_pip_options()
                + [make_spec("wheel", self.wheel_version)],
            )
    ...
    def create_archive(self, scratchpad, pkginfo, file_format):
        ...
        f = None
        try:
            if file_format in ("tar.gz", "tar.bz2", "tar"):
                if "." in file_format:
                    mode = "w:" + file_format.split(".")[1]
                else:
                    mode = "w"
                ...
                for dirpath, dirnames, files in os.walk(scratchpad):
                    for file_name in files:
                        f.write(
                            ...
                            os.path.join(base, dirpath[len(scratchpad) + 1:], file_name),
    ...
    def run_build_script(
            self, scratchpad, venv_path, build_script, install_script_path
    ):
    ...
    def finalize(self, artifact, elapsed_time):
        self.log.info("Done.")
        self.log.info("Total time elapsed: %.2fs" % elapsed_time)
    ...
    def build(self, file_format, prebuild_script=None, postbuild_script=None):
        if not os.path.isdir(self.path):
            raise click.UsageError("The project path (%s) does not exist" % self.path)
        ...
        if prebuild_script is not None:
            self.run_build_script(
                scratchpad, venv_path, prebuild_script, install_script_path
            )
        ...
        if postbuild_script is not None:
            self.run_build_script(
                scratchpad, venv_path, postbuild_script, install_script_path
            )

        if self.wheel_cache:
            self.update_wheel_cache(data_dir, venv_artifact)

        self.put_installer(scratchpad, pkginfo, install_script_path)
        artifact = self.create_archive(scratchpad, pkginfo, file_format)

        self.cleanup()
        self.finalize(artifact, time.time() - now)


@click.group(context_settings={"auto_envvar_prefix": "PLATTER"})
...
@click.option(
    "--virtualenv-version",
    help="The version of virtualenv to use. "
...
@click.option(
    "--wheel-version",
    help="The version of the wheel package "
...
@click.option(
    "--no-wheel-cache", is_flag=True, help="Disables the wheel cache entirely."
)
@click.option(
    "--no-download",
    is_flag=True,
...
def build_cmd(
    ...
    if no_wheel_cache:
        if no_download:
            raise click.UsageError(
                "--no-download and --no-cache cannot " "be used together."
            )
        wheel_cache = None
    ...
    with Builder(
        ...
        builder.build(
            file_format, prebuild_script=prebuild_script, postbuild_script=postbuild_script
        )
